...
`GET /stats` (`pv-api/src/routes/stats.js`) returns `totalPhotos`, `totalVideos`, `totalAlbums`, `totalSize`, and `photosByCountry` — rendered as a 2x2 tile grid plus a scrollable country list. It used to also return a raw per-extension breakdown and a giant per-album `.avif`-count table; both were dropped (no other consumer existed) because they dumped storage internals (`.json`/`.webp` artifact counts) at the user and the per-album table overflowed the popover. `totalPhotos`/`totalVideos` are counted with the same extension regexes `AlbumViewer.vue` uses to split its photo/video grids, and explicitly skip anything under `/thumbs/` — a thumbnail or metadata JSON is a storage artifact, not a photo. `totalSize`, by contrast, is the literal sum of every object's bytes (thumbnails included) since it's meant to answer "how much MinIO storage am I using," not "how many photos do I have."

`photosByCountry` reuses geodata pv-metadata already computed at upload time — no new reverse-geocoding calls. Each photo's entry in `<album>/<album>.json` (`media[]`) carries a `location` string from Mapbox's reverse-geocode (`pv-metadata/main.py`'s `reverse_geocode`, a `place_name` like `"Carrer Del Poeta Cabanyes 50, 08004 Barcelona, Barcelona, Spain"`), and the country is reliably its last comma-separated segment. Building this means one extra `GetObject` per album (its metadata JSON) beyond the existing bucket-wide listing — acceptable because `/stats` is fetched once per popover open, not polled. A photo whose `location` is `"not found"` (no GPS EXIF, no `MAPBOX_TOKEN` at upload time, or a failed geocode call) is skipped rather than counted under some "Unknown" bucket, and that's permanent for that photo — like the cover backfill, there's no re-geocoding pass for old data.
